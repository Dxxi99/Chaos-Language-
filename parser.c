...
static AstNode* parse_stmt() {
    AstNode* n;
    switch (current_token.type) {
        case TOK_NUM: case TOK_TEXT: case TOK_FLAG: case TOK_LIST:
            lexer_next(); n = calloc(1, sizeof(AstNode)); n->type = AST_VAR_DECL; n->var_decl.name = strdup(current_token.lexeme); expect(TOK_IDENT);
