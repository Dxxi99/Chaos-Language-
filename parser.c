...
is_mut = (current_token.type == TOK_VAL) ? 0 : 1;
        ...
        case TOK_IF: lexer_next(); n = calloc(1, sizeof(AstNode)); n->type = AST_IF; n->if_stmt.cond = parse_expr(); skip_nl(); n->if_stmt.then_body = parse_block(); if (current_token.type == TOK_ELSE) { lexer_next(); skip_nl(); if (current_token.type == TOK_IF) { n->if_stmt.else_body = parse_stmt(); } else { n->if_stmt.else_body = parse_block(); } } expect(TOK_END); expect(TOK_IF); return n;
