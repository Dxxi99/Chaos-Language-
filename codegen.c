...
void codegen_finish() { size_t len;char* ir=LLVMPrintModuleToString(mod);const char* id=LLVMGetModuleIdentifier(mod,&len);char fn[256];snprintf(fn,256,"%s.ll",id);FILE* f=fopen(fn,"w");fputs(ir,f);fclose(f);LLVMDisposeMessage(ir);LLVMDisposeBuilder(b);LLVMDisposeModule(mod);printf("Compiled: %s\n",fn); }
